        ...
        <div className="fixed top-20 right-4 z-[9999] flex flex-col gap-2 pointer-events-none">
            {toasts.map(t => (
                <div key={t.id} className="pointer-events-auto">
                    <NotificationToast item={t} onDismiss={() => onDismiss(t.id)} />
                </div>
            ))}
        </div>
    );
...
export default function NotificationCenter() {
    ...
    useEffect(() => {
        ...
        // Refresh notifications when chat is read
        const handleChatRead = () => loadNotifications();
        window.addEventListener('chat-read', handleChatRead);

        const interval = setInterval(loadNotifications, 7000); // Poll every 7 seconds
    ...
    return (
        <>
            {/* ── Toast Popups (outside dropdown DOM) ────────── */}
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-[350px] p-0 shadow-xl overflow-hidden rounded-xl border-primary/10">
                                ...
                                {notifications.map((n) => {
                                    ...
                                    return (
                                        <div
                                            key={n.id}
                                            className={cn(
                                                'p-4 border-b last:border-0 transition-colors cursor-default group relative',
