...
const bdeNavItems = [
  { icon: LayoutDashboard, label: 'Dashboard', path: '/bde' },
  { icon: Users, label: 'Clients', path: '/bde/clients' },
  ...
  { icon: Layout, label: 'Weekly Plan', path: '/bde/weekly-plan' },
  { icon: Award, label: 'My Performance', path: '/bde/performance' },
];
...
export default function BDELayout({ children, fullWidth = false }: BDELayoutProps) {
      ...
      <div className={cn(
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-white/80 backdrop-blur-md px-4 lg:px-8 shadow-sm shrink-0">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate('/bde/ai-assistant')}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="gap-2">
                ...
                <span className="hidden md:inline">{profile?.username || 'BDE'}</span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>My Account</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => navigate('/bde/profile')}>
