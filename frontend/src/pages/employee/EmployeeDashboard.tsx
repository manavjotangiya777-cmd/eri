...
import { CheckSquare, Clock, Calendar, Play, Pause, Coffee, LogOut, Timer, Bell, Cake, Sparkles } from 'lucide-react';
...
export default function EmployeeDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (attendanceData) {
        ...
        // If there's an open session (currently working), add elapsed live time
  ...
  // Effect to handle live timer increment
  useEffect(() => {
  ...
  const canBreakOut = () => {
    ...
    return diff >= 60; // 1-minute minimum break rule
  };
    ...
    <EmployeeLayout>
      <div className="space-y-6">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="mt-6 p-3 bg-muted/40 rounded-lg">
                <p className="text-[10px] text-muted-foreground italic leading-tight">
                  ...
                  Min. 1 minute session duration required for clock out.
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {/* Quick Actions moved to Dashboard for convenience */}
          <Card>
            <CardHeader>
              <CardTitle>Workplace Links</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-3">
              <Button variant="outline" className="justify-start gap-4 h-12" onClick={() => navigate('/employee/tasks')}>
                <CheckSquare className="h-5 w-5 text-primary" />
                <div className="text-left">
                  <div className="font-semibold text-sm">Task Board</div>
                  <div className="text-[10px] text-muted-foreground">Manage your assignments</div>
                </div>
              </Button>
              <Button variant="outline" className="justify-start gap-4 h-12" onClick={() => navigate('/employee/leave')}>
                <Calendar className="h-5 w-5 text-chart-2" />
                <div className="text-left">
                  <div className="font-semibold text-sm">Leave Management</div>
                  <div className="text-[10px] text-muted-foreground">Request time off</div>
                </div>
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-lg border-muted-foreground/10 overflow-hidden">
            ...
            <CardContent className="pt-6">
              {announcements.length === 0 ? (
                <div className="text-center py-10 text-muted-foreground text-sm italic border-2 border-dashed rounded-xl">
                  No new announcements today.
                </div>
              ) : (
                <div className="space-y-4">
                  {announcements.slice(0, 3).map((ann) => (
                    <div key={ann.id} className="p-4 rounded-xl bg-muted/30 border border-muted-foreground/10 hover:bg-muted/50 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-bold text-sm">{ann.title}</h4>
                        <Badge variant="outline" className="text-[9px] uppercase font-black">{ann.priority}</Badge>
                      </div>
          ...
          <Card className="shadow-2xl border-none bg-gradient-to-br from-pink-500/5 to-violet-500/5 overflow-hidden">
            <CardHeader className="bg-white/40 backdrop-blur-md border-b border-pink-500/10 pb-4">
              <CardTitle className="flex items-center justify-between text-xl">
                <div className="flex items-center gap-2 text-pink-600 font-extrabold tracking-tight">
                  <Cake className="h-6 w-6 animate-bounce" />
                  Upcoming Celebrations
                </div>
                {upcomingBirthdays.length > 0 && (
                  <Badge variant="outline" className="bg-pink-500/10 text-pink-600 border-pink-200 font-bold scale-90">
                    {upcomingBirthdays.length} Active
                  </Badge>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6 px-4">
              {upcomingBirthdays.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground bg-white/30 rounded-2xl border border-dashed border-pink-200">
                  <Sparkles className="h-10 w-10 mx-auto mb-3 opacity-20 text-pink-400" />
                  <p className="font-medium italic">No birthdays this month.</p>
                  <p className="text-[10px] mt-1 uppercase tracking-widest font-bold">Stay tuned for parties!</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {upcomingBirthdays.map((emp) => {
                    ...
                    return (
                      <div
                        key={emp.id}
                        className={cn(
                          "relative group flex items-center justify-between p-4 rounded-2xl transition-all duration-500",
                          isToday
                            ? "bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-lg shadow-pink-500/30 scale-105 z-10"
                            : "bg-white/60 hover:bg-white/80 border border-pink-500/5 hover:border-pink-500/20 shadow-sm"
                        )}
                      >
                        {isToday && (
                        ...
                        <div className="flex items-center gap-4">
                          <div className={cn(
                            "p-3 rounded-xl transition-transform group-hover:rotate-12 duration-300",
                            isToday ? "bg-white/20" : "bg-pink-500/10"
                          )}>
                            <Cake className={cn("h-5 w-5", isToday ? "text-white" : "text-pink-500")} />
                          </div>
                          <div>
                            <p className={cn("font-black text-sm tracking-tight", isToday ? "text-white" : "text-foreground")}>
                              {emp.full_name || emp.username}
                            </p>
                            <p className={cn("text-[10px] font-bold uppercase tracking-wider", isToday ? "text-pink-100" : "text-muted-foreground")}>
                              {new Date(emp.date_of_birth).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}
                            </p>
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                          <Badge className={cn(
                            "text-[10px] font-black px-3 py-0.5 rounded-full border-none",
                            isToday
                              ? "bg-white text-pink-600 shadow-md"
                              : "bg-pink-500/10 text-pink-600 shadow-none"
                          )}>
                            {isToday ? "PARTY TIME! 🎉" : `${emp.daysUntil} DAYS LEFT`}
                          </Badge>
                          {isToday && <span className="text-[8px] font-black italic tracking-tighter text-white animate-pulse">DON'T FORGET TO WISH!</span>}
                        </div>
                      </div>
                    );
                  })}
                  <p className="text-[10px] text-center text-muted-foreground font-bold uppercase tracking-[0.2em] pt-2 opacity-50">
