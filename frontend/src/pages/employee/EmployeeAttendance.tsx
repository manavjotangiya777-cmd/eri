...
export default function EmployeeAttendance() {
  ...
  const getCurrentStatus = () => {
    ...
    // Check if it's before work start time
    if (currentTimeStr < settings.work_start_time) {
      return {
        type: 'info',
        message: 'Work hasn\'t started yet',
        icon: Clock,
      };
    }

    // Check if it's lunch time
    if (currentTimeStr >= settings.lunch_start_time && currentTimeStr < settings.lunch_end_time) {
      return {
        type: 'lunch',
    ...
    // Check if it's after work end time
    if (currentTimeStr > settings.work_end_time) {
      return {
        type: 'overtime',
        message: 'Overtime Hours',
        icon: AlertCircle,
      };
    }
      ...
      <div className="space-y-6">
        ...
        {settings && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your daily work schedule and current time</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-4 border rounded-lg bg-card">
                  ...
                  <div className="text-xl font-bold text-primary">
                    {formatTimeOnly(settings.work_start_time)}
                  </div>
                ...
                <div className="p-4 border rounded-lg bg-card">
                  ...
                  <div className="text-xl font-bold">
                    {formatTimeOnly(settings.lunch_start_time)} - {formatTimeOnly(settings.lunch_end_time)}
                  </div>
                ...
                <div className="p-4 border rounded-lg bg-card">
                  ...
                  <div className="text-xl font-bold text-primary">
                    {formatTimeOnly(settings.work_end_time)}
                  </div>
                ...
                <div className="p-4 border rounded-lg bg-card">
                  ...
                  <div className="text-xl font-bold">
                    {settings.work_hours_per_day} hours
                  </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {todayAttendance ? 'Your attendance for today' : 'No attendance record for today'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
                ...
                <div className="p-4 border rounded-lg">
                  ...
                  <div className="text-2xl font-bold">
                    {todayAttendance?.actual_work_hours
                      ? `${todayAttendance.actual_work_hours.toFixed(2)}h`
                      : '-'}
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">Excluding lunch</div>
                </div>
              ...
              <TableBody>
                {attendanceHistory.map((record: Attendance) => (
                  <TableRow key={record.id}>
                    ...
                    <TableCell>
                      {record.actual_work_hours ? `${record.actual_work_hours.toFixed(2)}h` : '-'}
                    </TableCell>
                    <TableCell>
